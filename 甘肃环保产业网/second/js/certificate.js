...
//	page(true, true,true); //分页的所有效果js函数
//	传入name的值   true->全部的内容
//	传入nub的值   true->全部的证书
//	传入text的值   true->为空
...
//  
	...
	$(".dow_boxs4").click(function() {
		if(assLiClickFlag === true) {
			assLiClickFlag = false;
		   var cer1 = $('.dow_boxs1').find('option:selected').html();
		   var cer2 = $('.dow_boxs2').find('option:selected').html();
		   var cer3 = $("input[class='dow_boxs3']").attr("value")
...
(function(){
		...
		if(getUrlParams("nub")) {
			...
			var aname = getUrlParams("name");
            var anub = getUrlParams("nub");
            var atext = getUrlParams("text");
            if (aname=="公司名称") {
            	 $(".dow_boxs1 option[value='0']").attr("selected", "selected"); 
            } else if (aname=="证书内容"){
            	 $(".dow_boxs1 option[value='1']").attr("selected", "selected");
            } else if (aname=="证书ID"){
            	 $(".dow_boxs1 option[value='2']").attr("selected", "selected");
            }
            
            if (anub=="公司名称") {
            	 $(".dow_boxs2 option[value='0']").attr("selected", "selected"); 
            } else if (anub=="资质证"){
            	 $(".dow_boxs2 option[value='1']").attr("selected", "selected");
            } else if (anub=="设计证"){
            	 $(".dow_boxs2 option[value='2']").attr("selected", "selected");
            }else if (anub=="推荐证"){
            	 $(".dow_boxs2 option[value='3']").attr("selected", "selected");
            }
            $(".dow_boxs3").val(atext)
          
            page(aname, anub,atext); 
            pageIm();
            
		}
		if(getUrlParams("number")) {
			
            var number = getUrlParams("number");
...
function page(t, t1,t2) {
	...
	$.fn.pageFun = function(options) {
		...
		var Method = {
			init: function() {
				Method.getData(t, t1,t2); /*请求接口获得数据*/
				Method.handleEvent(); /*事件处理*/
			},

			getData: function(t, t1,t2) {
				$.ajax({
					type: "GET",
					url: opts.interFace,
					data: {
...
function pageIm() {
	...
	$(".pageBox").pageFun({ /*在本地服务器上才能访问哦*/
		...
		dataFun: function(data) {
			var dataHtml = "<ul><li>"+data.ID+"</li><li><a href='" + data.href + "'>" + data.company + "</a></li><li>" + data.type + "</li><li>" + data.content + "</li><li>" + data.time + "</li></ul>"
			return dataHtml;
		},
		pageFun: function(i) {
