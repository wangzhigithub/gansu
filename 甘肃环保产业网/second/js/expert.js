$(function(){
    ...
    page(true);
 //	传入type参数  true->全部    /0->水污染处理专家   / 1->  大气污染治理专家/ 2->噪声与振动治理专家 / 3->监测仪器专家 / 4->环评与工程设计专家 / 5->环评与工程设计专家  / 6->其他环保专家
   //  分页插件的请求数据
	$(".expert_qb .pageBox").pageFun({ /*在本地服务器上才能访问哦*/
		interFace: "../expert.json",
		/*接口*/
		displayCount: 15,
		/*每页显示总条数*/
		maxPage: 5,
		...
		dataFun: function(data) {
			var dataHtml = "<li><span><a href=''>"+data.name+"("+data.company+")</a></span><span>"+data.time+"</span></li>"
			return dataHtml;
		},
		pageFun: function(i) {
    ...
    $(".ass_ull li").click(function(){
    	...
    	if(assLiClickFlag === true) {
    	 ...
    	 $(".ass_ulr1").eq(i).css({display:"block"}).siblings().css({display:"none"})
	    	...
	    	if(i==0){
	    		$(".xiehui3").html("- 水污染处理专家")
	    	}
	    	if(i==1){
	    		$(".xiehui3").html("- 大气污染治理专家")
	    	}
	    	if(i==2){
	    		$(".xiehui3").html("- 噪声与振动治理专家")
	    	}
	    	if(i==3){
	    		$(".xiehui3").html("- 监测仪器专家")
	    	}
	    	if(i==4){
	    		$(".xiehui3").html("- 环评与工程设计专家")
	    	}
	    	if(i==5){
	    		$(".xiehui3").html("- 固体废物处理专家")
	    	}
	    	if(i==6){
	    		$(".xiehui3").html("- 其他环保专家")
    	    }
	    	
	    	$(".ass_ulr1 .pageDiv").html("");
			$(".ass_ulr1 .pageObj").html("");
			page(i);
		    pageIm(); 
...
function pageIm() {
	...
	$(".ass_ulr1 .pageBox").pageFun({ /*在本地服务器上才能访问哦*/
		interFace: "../expert1.json",
		/*接口*/
		displayCount: 15,
		/*每页显示总条数*/
		maxPage: 5,
